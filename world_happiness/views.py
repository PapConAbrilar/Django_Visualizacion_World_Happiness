...
def generosity_freedom(request):
    ...
    happy = paises[:30]
    unhappy = list(Pais.objects.all().order_by('happiness_score')[:30])
...
def agregar_pais_csv(request):
    if request.method == "POST":
        if "csv_file" not in request.FILES:
            return render(request, "world_happiness/agregar_pais_csv.html", {"error": "Debe seleccionar un archivo.","active_page":"agregar_pais_csv"})
        ...
        try:
            ...
            if not all(col in df.columns for col in columnas_esperadas):
                return render(request, "world_happiness/subir_csv.html", {
                    ...
                    "active_page":"agregar_pais_csv"
                })

            cargar_csv(df)
            return render(request, "world_happiness/agregar_pais_csv.html", {"msg": "Datos cargados correctamente.","active_page":"agregar_pais_csv"})

        except Exception as e:
            return render(request, "world_happiness/agregar_pais_csv.html", {"error": f"Error: {e}","active_page":"agregar_pais_csv"})
